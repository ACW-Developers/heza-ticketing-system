...
function Stat({ icon: Icon, label, value, hint, accent = "primary" }: any) {
  const tone = accent === "success" ? "bg-success/10 text-success"
    : accent === "accent" ? "bg-accent/30 text-accent-foreground"
...
function Dashboard() {
  ...
  const [stats, setStats] = useState({ events: 0, tickets: 0, revenue: 0, attendees: 0, upcoming: 0, checkedIn: 0 });
  ...
  useEffect(() => {
    (async () => {
      const [{ data: events }, { data: tickets }, { data: orders }] = await Promise.all([
        ...
        supabase.from("tickets").select("id, event_id, user_id, created_at, price, ticket_type, checked_in"),
      ...
      setStats({ events: events?.length ?? 0, tickets: tickets?.length ?? 0, revenue, attendees: uniq, upcoming, checkedIn });

      const byEvent: Record<string, { name: string; tickets: number; revenue: number }> = {};
      for (const e of events ?? []) byEvent[e.id] = { name: e.title.slice(0, 14), tickets: 0, revenue: 0 };
      for (const t of tickets ?? []) if (byEvent[t.event_id]) {
        byEvent[t.event_id].tickets++;
        byEvent[t.event_id].revenue += Number(t.price);
      }
      setChart(Object.values(byEvent).sort((a, b) => b.tickets - a.tickets).slice(0, 7));

      const days: any[] = [];
      for (let i = 13; i >= 0; i--) {
        const d = subDays(new Date(), i);
        const key = format(d, "MMM d");
        const dayTickets = (tickets ?? []).filter((t) => format(new Date(t.created_at), "MMM d") === key);
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Stat icon={Wallet} label="Revenue" value={fmt(stats.revenue, { decimals: 0 })} hint={`${stats.tickets} tickets sold`} accent="success" />
        <Stat icon={Ticket} label="Tickets" value={stats.tickets} hint={`Avg ${fmt(stats.tickets ? stats.revenue / stats.tickets : 0, { decimals: 0 })}`} />
        <Stat icon={Calendar} label="Events" value={stats.events} hint={`${stats.upcoming} upcoming`} accent="accent" />
        <Stat icon={Users} label="Attendees" value={stats.attendees} hint={`${checkinPct}% checked in`} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="h-72 mt-3">
            <ResponsiveContainer>
              <AreaChart data={trend} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <CartesianGrid strokeDasharray="4 6" stroke={GRID} strokeOpacity={0.3} vertical={false} />
                <XAxis dataKey="name" stroke="var(--color-muted-foreground)" fontSize={10} tickLine={false} axisLine={{ stroke: GRID, strokeOpacity: 0.4 }} />
                <YAxis stroke="var(--color-muted-foreground)" fontSize={11} tickLine={false} axisLine={{ stroke: GRID, strokeOpacity: 0.4 }} />
                <Tooltip contentStyle={TOOLTIP} />
                <Area type="monotone" dataKey="tickets" stroke={GREEN} strokeWidth={2.5} fill="url(#dashGreen)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="surface-card rounded-2xl p-6">
          ...
          <div className="h-72">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={typeMix} dataKey="value" nameKey="name" innerRadius={50} outerRadius={85} paddingAngle={3}>
                  {typeMix.map((_, i) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} stroke="var(--color-card)" strokeWidth={2} />)}
                </Pie>
                <Tooltip contentStyle={TOOLTIP} />
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={chart}>
                ...
                <XAxis dataKey="name" stroke="var(--color-muted-foreground)" fontSize={10} tickLine={false} axisLine={{ stroke: GRID, strokeOpacity: 0.4 }} />
                <YAxis stroke="var(--color-muted-foreground)" fontSize={11} tickLine={false} axisLine={{ stroke: GRID, strokeOpacity: 0.4 }} />
                <Tooltip contentStyle={TOOLTIP} cursor={{ fill: "var(--color-muted)", opacity: 0.3 }} />
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            <div className="relative h-44 w-44">
              <svg viewBox="0 0 120 120" className="h-full w-full -rotate-90">
                <circle cx="60" cy="60" r="50" stroke="var(--color-muted)" strokeWidth="12" fill="none" />
                <circle
                  cx="60" cy="60" r="50"
                  stroke={GREEN} strokeWidth="12" fill="none"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-xs text-muted-foreground mt-1 flex items-center gap-1"><CheckCircle2 className="h-3 w-3 text-success" /> attended</span>
              </div>
            </div>
            <div className="mt-4 text-sm text-muted-foreground">{stats.checkedIn} of {stats.tickets} tickets</div>
          </div>
        </div>
      </div>
    </div>
