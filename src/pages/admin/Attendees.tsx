...
function Attendees() {
  ...
  useEffect(() => {
    if (!eventId) return;
    supabase.from("tickets").select("*, orders(status, total_amount)").eq("event_id", eventId).order("created_at", { ascending: false })
  ...
  const byHour = useMemo(() => {
    ...
    return Object.entries(buckets).map(([name, value]) => ({ name, value })).slice(-14);
  }, [tickets]);

  function downloadCSV() {
    const event = events.find((e) => e.id === eventId);
    const headers = ["#", "Ticket", "Type", "Name", "Email", "Phone", "ID/Passport", "Country", "Gender", "Price", "Checked in", "Created"];
    const rows = tickets.map((t, i) => [
      i + 1, t.ticket_number, t.ticket_type, t.attendee_name ?? "", t.attendee_email ?? "",
      t.attendee_phone ?? "", t.id_number ?? "", t.country ?? "", t.gender ?? "",
      t.price, t.checked_in ? "Yes" : "No", format(new Date(t.created_at), "yyyy-MM-dd HH:mm"),
    ]);
    rows.push([] as any, ["TOTAL", String(tickets.length)] as any, ["REVENUE", fmt(stats.revenue)] as any);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3 justify-between">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button onClick={downloadCSV} disabled={!tickets.length}><Download className="mr-1 h-4 w-4" /> CSV</Button>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Kpi icon={Users} label="Tickets sold" value={stats.total} />
        <Kpi icon={CheckCircle2} label="Checked in" value={stats.checkedIn} hint={`${stats.total ? Math.round((stats.checkedIn / stats.total) * 100) : 0}%`} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <ChartCard title="Top countries">
          <ResponsiveContainer>
            <BarChart data={byCountry} layout="vertical" margin={{ left: 80 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
      ...
      <div className="surface-card rounded-2xl overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-left text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              <th className="p-3">Ticket #</th><th className="p-3">Attendee</th>
              <th className="p-3">Contact</th><th className="p-3">ID / Country</th>
              <th className="p-3">Type</th><th className="p-3">Price</th>
            ...
            {tickets.map((t) => (
              <tr key={t.id} className="border-t border-border hover:bg-muted/20">
                ...
                <td className="p-3">{t.checked_in
                  ...
                  : <span className="rounded-full bg-muted text-muted-foreground px-2 py-0.5 text-xs">Pending</span>}
                </td>
              </tr>
            ))}
            {tickets.length === 0 && <tr><td colSpan={7} className="p-10 text-center text-muted-foreground">No tickets sold yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
