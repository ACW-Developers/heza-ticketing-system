...
function Reports() {
  ...
  function exportEventPDF() {
    ...
    const doc = new jsPDF();
    doc.setFontSize(22);
    doc.text("Event Attendance List", 14, 18);
    doc.setFontSize(14);
    doc.setTextColor(60);
    doc.text(ev.title, 14, 27);
    doc.setFontSize(10);
    doc.setTextColor(120);
    doc.text(
      `${ev.venue} · ${format(new Date(ev.event_date), "EEEE MMM d, yyyy · h:mm a")}`,
      14,
      33,
    );
    doc.setTextColor(0);
    ...
    autoTable(doc, {
      ...
      body: tickets.map((t, i) => [
        i + 1,
        t.ticket_number,
        String(t.ticket_type).toUpperCase(),
        t.attendee_name || "—",
        t.attendee_email || "—",
        t.attendee_phone || "—",
        t.id_number || "—",
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Generate sales summary and per-event attendance lists
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          {picked === "all" ? (
            <Button onClick={exportOverallPDF}>
              <FileDown className="mr-1.5 h-4 w-4" /> Sales PDF
            </Button>
          ) : (
            <Button onClick={exportEventPDF}>
              <Download className="mr-1.5 h-4 w-4" /> Attendance PDF
            </Button>
          )}
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <KpiBox
              label="Revenue"
              value={fmt(
                tickets.reduce((s, t) => s + Number(t.price), 0),
                { decimals: 0 },
              )}
            />
          </div>
